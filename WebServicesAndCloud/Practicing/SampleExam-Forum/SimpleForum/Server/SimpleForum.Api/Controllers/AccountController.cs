namespace SimpleForum.Api.Controllers
{
    ...
    using SimpleForum.Api.Results;
    using SimpleForum.Models;
    using SimpleForum.Api.Providers;
    ...
    public class AccountController : ApiController
    {
        ...
        public async Task<IHttpActionResult> Register(RegisterBindingModel model)
        {
            ...
            var user = new User() { UserName = model.Username, Email = model.Nickname + "@mydomain.bg" };

            IdentityResult result = await this.UserManager.CreateAsync(user, model.AuthCode);
        ...
        public async Task<IHttpActionResult> GetExternalLogin(string provider, string error = null)
        {
            if (error != null)
            {
                return this.Redirect(string.Format("{0}#error={1}", this.Url.Content("~/"), Uri.EscapeDataString(error)));
            }
