namespace JediMeditation
{
    ...
    public class Program
    {
        ...
        {
            int n = int.Parse(GetInputData());

            var members = GetInputData();

            var jedis = SplitMembers(members);

            var priorityQueue = new PriorityQueue<int, string>(n, new MyComparer());
            ...
            for (int i = 0; i < n; i++)
            {
                if (jedis[i][0] == 'm')
                {
                   priorityQueue.Enqueue(0 + i, jedis[i]);
                }
                else if (jedis[i][0] == 'k')
                {
                    priorityQueue.Enqueue(100001 + i, jedis[i]);
                }
                else
                {
                    priorityQueue.Enqueue(200001 + i, jedis[i]);
                }
            }

            while (priorityQueue.Count > 0)
            {
                Console.Write(priorityQueue.PeekValue());
                Console.Write(" ");
                priorityQueue.Dequeue();
            }
        }

        private static string[] SplitMembers(string members)
        {
